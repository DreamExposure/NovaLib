package org.dreamexposure.novalib.api.bukkit.compatibility;
...
public enum NMaterial {
    //Items in alphabetical order --- 1.14 names are used for the enum value
    ...
    ACACIA_SIGN("SIGN"),
    ACACIA_SLAB("WOOD_SLAB:4"),
    ACACIA_STAIRS("WOOD_STAIRS:4"),
    ...
    CHISELED_QUARTS_BLOCK("QUARTZ_BLOCK:1"),
    ...
    CRACKED_STONE_BRICKS("SMOOTH_BRICKS:2"),
    ...
    GREEN_DYE("CACTUS_GREEN", "INK_SAC:2", "INK_SACK:2"),
    ...
    LIGHT_BLUE_DYE("INK_SAC:12", "INK_SACK:12"),
    ...
    LIGHT_GRAY_DYE("INK_SAC:7", "INK_SACK:7"),
    ...
    LIME_DYE("INK_SAC:10", "INK_SACK:10"),
    ...
    MAGENTA_DYE("INK_SAC:13", "INK_SACK:13"),
    ...
    MOSSY_STONE_BRICKS("SMOOTH_BRICKS:1"),
    ...
    ORANGE_DYE("INK_SAC:14", "INK_SACK:14"),
    ...
    PURPLE_DYE("INK_SAC:5", "INK_SACK:5"),
    ...
    RED_CONCRETE_POWDER("CONCRETE:POWDER:14"),
    ...
    RED_STAINED_GLASS_PANE("STAINED_GLASS:14"),
    ...
    YELLOW_DYE("DANDELION_YELLOW", "INK_SAC:11", "INK_SACK:11"),
    ...
    ZOMBIE_WALL_HEAD("MONSTER_EGG");

    String[] names;

    NMaterial(String... _names) {
        names = _names;
    }

    public Material getBukkitMaterial() {
        if (Material.matchMaterial(this.name()) != null)
    ...
    public ItemStack getItemStack() {
        if (Material.matchMaterial(this.name()) != null)
            return new ItemStack(Material.matchMaterial(name()), 1);

        for (String s : names) {
            Material m = Material.AIR;
            short data = 0;
            if (s.contains(":")) {
                data = Short.valueOf(s.split(":")[1]);
                s = s.split(":")[0];
            }
            if (Material.matchMaterial(s) != null)
        ...
        return new ItemStack(Material.AIR); //This way we aren't sending NPEs everywhere
    }

    public static NMaterial getNMaterial(String checkFor) {

        //Check enum name values
        for (NMaterial nm : values()) {
            if (checkFor.equalsIgnoreCase(nm.name()))
                return nm;
        }

        //Check old names and bukkit materials
        for (NMaterial nm : values()) {
            if (nm.getBukkitMaterial().name().equalsIgnoreCase(checkFor))
                return nm;

            for (String n : nm.names) {
                if (n.equalsIgnoreCase(checkFor))
                    return nm;
            }
        }

        //Doesn't exist, as new or old name. Return air.
