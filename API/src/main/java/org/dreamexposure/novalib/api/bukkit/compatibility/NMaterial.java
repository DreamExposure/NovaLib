...
public enum NMaterial {
    ...
    public boolean isSameMaterial(ItemStack comp) {
        ...
        if (comp.getType() == parseMaterial() && (int) comp.getData().getData() == data)
            return true;
        NMaterial nMaterial = fromMaterial(comp.getType());
        if (isDamageable(nMaterial))
            return parseMaterial() == comp.getType();
        return false;
    }
    
    public NMaterial fromMaterial(Material mat) {
        try {
            return NMaterial.valueOf(mat.toString());
        } catch (IllegalArgumentException e) {
            for (NMaterial nMaterial : NMaterial.values()) {
                if (nMaterial.m.equals(mat.toString())) {
                    return nMaterial;
                }
            }
        }
    ...
    public static NMaterial fromString(String key) {
        NMaterial nMaterial;
        try {
            nMaterial = NMaterial.valueOf(key);
            return nMaterial;
        } catch (IllegalArgumentException e) {
    ...
    public boolean isDamageable(NMaterial type) {
